...
class Renderable
{
    ...
    protected:
        ...
        template <typename T>
        void renderIndicesAsTrianglesInstanced(const IndexBuffer<T> &buffer, const size_t &nrInstances) const
        {
            if (nrInstances > 0)
            {
                buffer.bind();
                GL_CHECK(glDrawElementsInstanced(GL_TRIANGLES, buffer.size(), detail::getOpenGLDataType<T>(), 0, nrInstances));
                buffer.unbind();
            }
        }
